...
function  edgesOrder(polygon){
  // https://stackoverflow.com/questions/45660743/sort-points-in-counter-clockwise-in-javascript
}



function generate10Transect (referenceTransect, distanceBetweenLines, refAngle){
  var result = [];
  var transect = referenceTransect;
  var tempAngle = refAngle;
  console.log(" reference transect (or transect 0) : ");
  console.log(transect);
  console.log(tempAngle);

  //MD : 04.02.21 : Check in which direction the transect should go
  if ((tempAngle > -45 && tempAngle < 45) || tempAngle > 135 && tempAngle < 215){
    console.log("translation horizontale");

    var refYP1 = transect[0][1] + (distanceBetweenLines/2);
    var refYP2 = transect[1][1] + (distanceBetweenLines/2);

    result[0] = [[transect[0][0], refYP1], [transect[1][0], refYP2]];

      for (var i = 1; i < 10; i++) {
          result[i] = [[transect[0][0], (refYP1 + (distanceBetweenLines * (i)))], [transect[1][0], (refYP2 + (distanceBetweenLines * (i)))]];
      }
      console.log(" 10 transects generated : ");
      console.log(result);
  }else{
    console.log("translation verticale");
    var refXP1 = transect[0][0] + (distanceBetweenLines/2);
    var refXP2 = transect[1][0] + (distanceBetweenLines/2);

    result[0] = [[refXP1, transect[0][1]], [refXP2, transect[1][1]]];

      for (var i = 1; i < 10; i++) {
          result[i] = [[(refXP1 + (distanceBetweenLines * (i))), transect[0][1]], [(refXP2 + (distanceBetweenLines * (i))), transect[1][1]]];
      }
      console.log(" 10 transects generated : ");
      console.log(result);
